...
---
# <a name="setlocalvar-macro-action-access-custom-web-app"></a>予約マクロ アクション (カスタム web アプリケーションのアクセス)
...
> [!NOTE]
> [!メモ] " **SetLocalVar/ローカル変数の設定** " アクションは、データ マクロでのみ使用できます。 
...
|**Expression** <br/> |はい  <br/> |この一時変数の値を設定に使用する式を指定します。 式の前に等号 (=) を付けないでください。 この引数を設定するのには、**式ビルダー**を使用するのには [**ビルド**] ボタンをクリックすることができます。  <br/> |
