...
---
# <a name="databasecreateproperty-method-dao"></a>CreateProperty メソッド (DAO)
...
<td><p>Optional</p></td>
<td><p><strong>Variant</strong></p></td>
<td><p><strong>Property</strong> が DDL オブジェクトであるかどうかを示す、サブタイプがブール型 (<strong>Boolean</strong>) であるバリアント型 (<strong>Variant</strong>) の値。 既定は False です。 DDL が True の場合、ユーザーは、dbsecwritedef 権限を持っていない限り、この<strong>プロパティ</strong>オブジェクトを変更または削除することはできません。</p></td>
</tr>
</tbody>
</table>
...
> [!NOTE]
> ddl 引数を省略すると、既定では False (非 DDL) になります。 対応する DDL プロパティは公開されていないので、DDL から DDL 以外に変更する **Property** オブジェクトを削除し、再作成する必要があります。
