...
> [!NOTE]
> 文字列、整数以外の値に連結するプロパティを設定して、システム ・ パラメーターは、米国以外の小数点の記号、カンマなどを指定する場合 (たとえば、strSQL ="価格&gt;" &amp; lngPrice でと lngPrice = 125,50)、エラーが発生とします。Microsoft Access データベース エンジン データベースの**クエリ定義**オブジェクトを実行しようとしてください。 連結時に数値がシステムの既定の小数点の記号を使って文字列に変換されますが、Microsoft Access の SQL で小数点の記号として使用できるのはピリオドのみであるためです。
