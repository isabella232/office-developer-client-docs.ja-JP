...
次の使用例では、[レコード セット](recordset-object-ado.md)オブジェクトの[Find](find-method-ado.md)メソッドを使用して、G. カットの文字で始まる名前、 ***Northwind***データベース内の会社を表示し、メモ帳または別のテキスト エディターに次のコードを貼り付けし、**として保存するにはFindJS.asp**。
