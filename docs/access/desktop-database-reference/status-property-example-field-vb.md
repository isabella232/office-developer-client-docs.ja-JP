...
次の例では、ドキュメントから開いている **Record** から既知の **Field** を削除します。 **Status** プロパティは、最初は **adFieldOK** に設定され、次に **adFieldPendingUnknown** に設定されます。
...
次のコードは、読み取り専用のドキュメントで開いている **Record** から **Field** を削除します。 **Status** は **adFieldPendingDelete** に設定されます。 [Update](update-method-ado.md) を呼び出すと、削除は失敗し、 **Status** は **adFieldPendingDelete** および **adFieldPermissionDenied** になります。 [CancelUpdate](cancelupdate-method-ado.md) によって保留状態の **Status** の設定がクリアされます。
