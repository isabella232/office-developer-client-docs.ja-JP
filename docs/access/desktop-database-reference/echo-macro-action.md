...
によってマクロが実行されているとき、画面の更新によって、マクロの機能には必要ない情報が表示されることはよくあります。 **Echo On/エコーの設定** 引数を [ **いいえ** ] に設定すると、画面を更新せずにマクロが実行されます。マクロが完了すると、エコーが自動的にオンになり、ウィンドウが再描画されます。 **Echo On/エコーの設定** 引数を [ **いいえ** ] に設定しても、マクロの機能またはその結果には影響しません。
