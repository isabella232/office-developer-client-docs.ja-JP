...
データ クエリ [Recordset](recordset-object-ado.md) を 1 つまたは複数のコントロール (テキスト ボックス、グリッド コントロール、コンボ ボックスなど) とバインドして、Web ページに **Recordset** データを表示します。
...
OBJECT タグ経由で 1 つの Web ページに組み込むことができる **RDS.DataControl** オブジェクトの数に制限はありません。

Web ページ上の **RDS.DataControl** オブジェクトを定義する場合、 **Height** と **Width** の値として、1 などの 0 以外の値を指定します (余分な空白を含めないため)。
