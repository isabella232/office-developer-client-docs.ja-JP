...
<td><p>Either the source URL or the parent of the destination URL does not exist. There is a typographical error in either the source or destination URL. 代わりに実際 https://mysite/photo/myphoto.jpg に持つ必要がある場合 https://mysite/photos/myphoto.jpg があります。 親 URL が誤入力されていることにより (この例では <em></em>photos ではなく <em></em>photo と指定したことにより) エラーが発生しています。</p></td>
</tr>
<tr class="odd">
