---
title: Recordset のメソッド (DAO)
TOCTitle: Methods
ms:assetid: 8b713eda-b076-4190-b2f5-ff1ce522e2bf
ms:mtpsurl: https://msdn.microsoft.com/library/Dn125237(v=office.15)
ms:contentKeyID: 52073361
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: 696d3fe0d1b9bf67df3f12608f4bd7ba560d0dae
ms.sourcegitcommit: d7248f803002b31cf7fc561b03530199a9b0a8fd
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 11/02/2018
ms.locfileid: "25920754"
---
# <a name="recordset-methods-dao"></a>Recordset のメソッド (DAO)


**適用されます**Access 2013、Office 2013。



  - [Recordset.AddNew メソッド (DAO)](recordset-addnew-method-dao.md)

  - [Recordset.Cancel メソッド (DAO)](recordset-cancel-method-dao.md)

  - [Recordset.CancelUpdate メソッド (DAO)](recordset-cancelupdate-method-dao.md)

  - [Recordset.Clone メソッド (DAO)](recordset-clone-method-dao.md)

  - [Recordset.Close メソッド (DAO)](recordset-close-method-dao.md)

  - [Recordset.CopyQueryDef メソッド (DAO)](recordset-copyquerydef-method-dao.md)

  - [Recordset.Delete メソッド (DAO)](recordset-delete-method-dao.md)

  - [Recordset.Edit メソッド (DAO)](recordset-edit-method-dao.md)

  - [Recordset.FillCache メソッド (DAO)](recordset-fillcache-method-dao.md)

  - [Recordset.FindFirst メソッド (DAO)](recordset-findfirst-method-dao.md)

  - [Recordset.FindLast メソッド (DAO)](recordset-findlast-method-dao.md)

  - [Recordset.FindNext メソッド (DAO)](recordset-findnext-method-dao.md)

  - [Recordset.FindPrevious メソッド (DAO)](recordset-findprevious-method-dao.md)

  - [Recordset.GetRows メソッド (DAO)](recordset-getrows-method-dao.md)

  - [Recordset.Move メソッド (DAO)](recordset-move-method-dao.md)

  - [Recordset.MoveFirst メソッド (DAO)](recordset-movefirst-method-dao.md)

  - [Recordset.MoveLast メソッド (DAO)](recordset-movelast-method-dao.md)

  - [Recordset.MoveNext メソッド (DAO)](recordset-movenext-method-dao.md)

  - [Recordset.MovePrevious メソッド (DAO)](recordset-moveprevious-method-dao.md)

  - [Recordset.NextRecordset メソッド (DAO)](recordset-nextrecordset-method-dao.md)

  - [Recordset.OpenRecordset メソッド (DAO)](recordset-openrecordset-method-dao.md)

  - [Recordset.Requery メソッド (DAO)](recordset-requery-method-dao.md)

  - [Recordset.Seek メソッド (DAO)](recordset-seek-method-dao.md)

  - [Recordset.Update メソッド (DAO)](recordset-update-method-dao.md)

