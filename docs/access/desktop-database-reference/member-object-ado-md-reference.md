---
title: Member オブジェクト (ADO MD の参照)
TOCTitle: Member object (ADO MD)
ms:assetid: 933be014-9cb2-416a-8290-6dece686fc41
ms:mtpsurl: https://msdn.microsoft.com/library/JJ249650(v=office.15)
ms:contentKeyID: 48546393
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: 28e1b6bf24f55e9e4ad52730f223509542e22f58
ms.sourcegitcommit: d7248f803002b31cf7fc561b03530199a9b0a8fd
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 11/02/2018
ms.locfileid: "25920894"
---
# <a name="member-object-ado-md-reference"></a>Member オブジェクト (ADO MD の参照)

**適用されます**Access 2013、Office 2013。

- [Member オブジェクト (ADO MD)](member-object-ado-md.md)
- [メンバー プロパティ、メソッド、およびイベント (ADO MD)](member-properties-methods-and-events-ado-md.md)

