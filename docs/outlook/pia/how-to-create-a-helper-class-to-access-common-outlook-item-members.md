...
ms.locfileid: "32359403"
---
# <a name="create-a-helper-class-to-access-common-outlook-item-members"></a><span data-ttu-id="a7b5d-102">ヘルパー クラスを作成し、Outlook アイテムの共通メンバーにアクセスする</span><span class="sxs-lookup"><span data-stu-id="a7b5d-102">Create a Helper class to access common Outlook item members</span></span>

<span data-ttu-id="a7b5d-103">この例では、Outlook アイテム オブジェクトの共通のプロパティとメソッドにアクセスする OutlookItem ヘルパー クラスを実装し、これらの共通アイテムのメンバーにアクセスする前に、特定のアイテム オブジェクトをテストしてキャストするオーバーヘッドを削減する方法を示します。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-103">This example shows how to implement an OutlookItem helper class that accesses common properties and methods of Outlook item objects, saving the overhead of testing for and casting to a specific item object before accessing these common item members.</span></span>

## <a name="example"></a><span data-ttu-id="a7b5d-104">例</span><span class="sxs-lookup"><span data-stu-id="a7b5d-104">Example</span></span>

> [!NOTE] 
> <span data-ttu-id="a7b5d-105">次のコード例は、『[Programming Applications for Microsoft Office Outlook 2007](https://www.amazon.com/gp/product/0735622493?ie=UTF8&tag=msmsdn-20&linkCode=as2&camp=1789&creative=9325&creativeASIN=0735622493)』からの抜粋です。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-105">The following code example is an excerpt from [Programming Applications for Microsoft Office Outlook 2007](https://www.amazon.com/gp/product/0735622493?ie=UTF8&tag=msmsdn-20&linkCode=as2&camp=1789&creative=9325&creativeASIN=0735622493).</span></span>


<span data-ttu-id="a7b5d-106">多くの Outlook アイテムに、同様のプロパティとメソッドがあります。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-106">Many Outlook items have similar properties and methods.</span></span> <span data-ttu-id="a7b5d-107">たとえば、**Application**、**Attachments**、**Body**、**Categories**、および **Class** の各プロパティと、**Close**、**Copy**、および **Display** の各メソッドは、すべての Outlook アイテムのオブジェクトに共通して存在します。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-107">For example, the **Application**, **Attachments**, **Body**, **Categories**, and **Class** properties, and **Close**, **Copy**, and **Display** methods are common to all Outlook item objects.</span></span> <span data-ttu-id="a7b5d-108">COM ベースの Outlook オブジェクト モデルは、多くのメンバーについて厳密なアイテムの種類ではなく、汎用の Visual Basic **Object** を返します。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-108">The COM-based Outlook object model returns the generic Visual Basic **Object** instead of the exact item type for many members.</span></span> 

<span data-ttu-id="a7b5d-109">たとえば、 [CurrentItem](https://msdn.microsoft.com/library/bb611743\(v=office.15\)) プロパティは、ジェネリック **Object** を返します。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-109">For example, the [CurrentItem](https://msdn.microsoft.com/library/bb611743\(v=office.15\)) property returns a generic **Object**.</span></span> <span data-ttu-id="a7b5d-110">その一方で、厳密に型指定されたマネージ コード環境では、Outlook アイテムを表す **Object** を **MailItem** などの Outlook の正確な種類にキャストする必要があります。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-110">On the other hand, the strongly typed managed code environment requires you to cast the **Object** representing an Outlook item to the exact Outlook type such as **MailItem**.</span></span> 

<span data-ttu-id="a7b5d-111">OutlookItem ヘルパー クラスは、リフレクションを使用して、すべてのアイテムに共通するプロパティとメソッドを公開します。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-111">The OutlookItem helper class uses reflection to expose properties and methods that are common to all items.</span></span> <span data-ttu-id="a7b5d-112">このクラスは、オブジェクトを厳密な型にキャストする際に役立ち、**OutlookItem** オブジェクトで共通するアイテムのプロパティまたはメソッドを直接使用できる利便性を提供します。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-112">The class helps you cast the object to the exact type and provides the convenience of directly using common item properties or methods on the **OutlookItem** object.</span></span> <span data-ttu-id="a7b5d-113">これはとても便利な手法であり、「**関連項目**」セクションに記載されている方法についての各トピックでも、このヘルパー クラスが利用されています。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-113">This is a very useful technique, and several other how-to topics listed in the **See also** section take advantage of this helper class.</span></span>

<span data-ttu-id="a7b5d-114">Visual Studio を使用してこのコード例をテストする場合、**Microsoft.Office.Interop.Outlook** 名前空間をインポートするときに、まず Microsoft Outlook 15.0 オブジェクト ライブラリ コンポーネントへの参照を追加し、Outlook 変数を指定します。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-114">If you use Visual Studio to test this code example, you must first add a reference to the Microsoft Outlook 15.0 Object Library component and specify the Outlook variable when you import the **Microsoft.Office.Interop.Outlook** namespace.</span></span> <span data-ttu-id="a7b5d-115">**Imports** または **using** ステートメントは、コード例の関数の前に直接置くことはできません。パブリック Class 宣言の前に追加する必要があります。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-115">The **Imports** or **using** statement must not occur directly before the functions in the code example but must be added before the public Class declaration.</span></span> <span data-ttu-id="a7b5d-116">次のコード行は、Visual Basic および C\# でインポートおよび割り当てを行う方法を示しています。</span><span class="sxs-lookup"><span data-stu-id="a7b5d-116">The following lines of code show how to do the import and assignment in Visual Basic and C\#.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="a7b5d-117">関連項目</span><span class="sxs-lookup"><span data-stu-id="a7b5d-117">See also</span></span>

- [<span data-ttu-id="a7b5d-118">アクティブ エクスプローラーで選択した項目を表示する</span><span class="sxs-lookup"><span data-stu-id="a7b5d-118">Display selected items in the active Explorer</span></span>](how-to-display-selected-items-in-the-active-explorer.md)
- [<span data-ttu-id="a7b5d-119">iCalendar ファイルを開いて内容を表示する</span><span class="sxs-lookup"><span data-stu-id="a7b5d-119">Open and display the contents of an iCalendar file</span></span>](how-to-open-and-display-the-contents-of-an-icalendar-file.md)
- [<span data-ttu-id="a7b5d-120">分類項目をアイテムに割り当てる</span><span class="sxs-lookup"><span data-stu-id="a7b5d-120">Assign categories to an item</span></span>](how-to-assign-categories-to-an-item.md)
- [<span data-ttu-id="a7b5d-121">インスペクターのラッパーを実装し、インスペクターごとにアイテム レベルのイベントを追跡する</span><span class="sxs-lookup"><span data-stu-id="a7b5d-121">Implement a wrapper for inspectors and track item-level events in each inspector</span></span>](how-to-implement-a-wrapper-for-inspectors-and-track-item-level-events-in-each-inspector.md)
- [<span data-ttu-id="a7b5d-122">Outlook アイテム全般</span><span class="sxs-lookup"><span data-stu-id="a7b5d-122">General Outlook items</span></span>](general-outlook-items.md)
