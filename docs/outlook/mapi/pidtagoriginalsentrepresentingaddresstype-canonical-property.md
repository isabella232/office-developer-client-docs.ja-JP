...
元のメッセージが送信されたメッセージング ユーザーのアドレスの種類を格納します。
...
これらのプロパティは、メッセージの元の送信者の種類です。 これらは会話スレッドで使用されます。
  
別のクライアントに代わってメッセージを送信するクライアント アプリケーションは、これらのプロパティを、メッセージの最初の送信時に **PR_SENT_REPRESENTING_ADDRTYPE** ([PidTagSentRepresentingAddressType](pidtagsentrepresentingaddresstype-canonical-property.md)) プロパティの値に設定する必要があります。 一度設定した場合は、変更する必要はありません。
