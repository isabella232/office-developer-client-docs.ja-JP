...
メッセージングシステムがこのメッセージの読み取りレポートを送信するメッセージングユーザーのエントリ識別子を含みます。
...
クライアントアプリケーションが読み取りレポート自体を受信する必要がある場合は、このプロパティを未設定のままにするか、またはメッセージの送信時に**PR_SENDER_ENTRYID** ([PidTagSenderEntryId](pidtagsenderentryid-canonical-property.md)) プロパティに含まれるエントリ識別子に設定します。
