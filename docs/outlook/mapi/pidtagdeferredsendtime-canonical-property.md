...
クライアントがメッセージの送信を延期しようとしている時間を示します。
...
**PR_DEFERRED_SEND_UNITS** ([PidTagDeferredSendUnits](pidtagdeferredsendunits-canonical-property.md)) および**PR_DEFERRED_SEND_NUMBER** ([PidTagDeferredSendNumber](pidtagdeferredsendnumber-canonical-property.md)) プロパティが存在する場合、このプロパティの値は次の数式を使用して再計算されます。古い値は無視されます。
...
**PR_DEFERRED_SEND_TIME** value が現在の時刻 (UTC) よりも早い場合、メッセージは直ちに送信されます。 
