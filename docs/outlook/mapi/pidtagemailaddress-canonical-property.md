...
これらのプロパティは、すべてのメッセージングユーザーのベースアドレスプロパティの例です。 この文字列は、元になるメッセージングシステムに対してのみ書式が設定された、null で終了する文字列です。 
  
これらのプロパティは、メッセージのアドレス指定で**PR_ADDRTYPE** ([PidTagAddressType](pidtagaddresstype-canonical-property.md)) および**PR_MESSAGE_CLASS** ([PidTagMessageClass](pidtagmessageclass-canonical-property.md)) プロパティと組み合わせて使用されます。 文字列の形式は、 **PR_ADDRTYPE**によって修飾されます。 
