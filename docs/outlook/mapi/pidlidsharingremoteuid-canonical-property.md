...
共有するリモート フォルダーのエントリ ID を指定します。 これは、共有メッセージのプロパティです。
...
このプロパティは、共有するフォルダーの PR_ENTRYID ([PidTagEntryId](pidtagentryid-canonical-property.md)) プロパティの値の 16 進文字列表記に設定する必要があります。 これは、共有メッセージのプロパティです。
