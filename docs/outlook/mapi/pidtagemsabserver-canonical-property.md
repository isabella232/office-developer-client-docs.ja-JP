...
オフラインシナリオのアドレス帳コンテナーのパス、またはアドレス帳コンテナーが存在するグローバルカタログサーバーの完全修飾ドメイン名のどちらかをオンラインシナリオで指定します。
...
このプロパティでは、UNICODE プラットフォームで**** `UNICODE`シンボルをコンパイルしたときにプロパティの種類が PT_UNICODE にリセットさ**** れ、 `UNICODE`シンボルを使用してコンパイルされていない場合は PT_STRING8 になります。 
